 ...
 */
const EXTENSION_NAME = 'sinag/hide';
			...
			return (
				<Fragment>
					...
					<InspectorControls group="styles">
						<PanelBody
							title={__('Responsive Visibility', 'sinag')}
							initialOpen={false}
						>
							<ButtonGroup>
...
const modifySave = (props, block, attributes) => {
	...
	if (isAffected(name)) {
		...
		if (attributes.sinagClassNameHide) {
			return {
				...props,
				className: classnames(
					props.className,
					attributes.sinagClassNameHide.map((opt) => {
						return opt.value;
					})
				),
			};
