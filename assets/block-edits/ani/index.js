 ...
 */
const Validation = ({ sinagClassNameAni }) => {
	// start without a message
	const msgs = [];
	// get the animations the user selected
	const chosen = sinagClassNameAni.map((aniPair) =>
	...
	[
		...
		if (!check.includes(false)) {
			msgs.push(
				`${__('animations', 'sinag')} ${set.join(', ')} ${__(
	...
	(BlockEdit) => (props) => {
		...
		if (isAffected(name)) {
			...
			const [selectedStrings, setSelectedStrings] = useState(
				[...sinagClassNameAni].map((pair) => pair.title)
			);
			...
			const handleChange = (tokens) => {
				...
				setAttributes({ sinagClassNameAni: newAttributes });
			};
			...
			return (
				<Fragment>
					...
					<InspectorControls group="styles">
						<PanelBody title={__('Animations', 'sinag')}>
							<FormTokenField
								value={selectedStrings}
								label={__('Properties to animate', 'sinag')}
							...
							/>
							<Validation
								sinagClassNameAni={sinagClassNameAni}
							/>
						</PanelBody>
					</InspectorControls>
				</Fragment>
...
const modifySave = (props, block, attributes) => {
	...
	if (isAffected(name)) {
		...
		if (attributes.sinagClassNameAni) {
			const existingClasses = props.className
				? props.className.split(' ')
				: [];
			const newClasses = attributes.sinagClassNameAni
				.map((opt) => opt.value)
