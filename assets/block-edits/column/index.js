 ...
 */
const EXTENSION_NAME = 'sinag/column';
...
const withModifyEdit = createHigherOrderComponent(
	...
	(BlockEdit) => (props) => {
		...
		if (isAffected(name)) {
			...
			const {
				attributes: { sinagClassNameColumn },
				setAttributes,
			} = props;
			...
			const handleChange = (tokens) => {
				...
				setAttributes({ sinagClassNameColumn: newAttributes });
			};
			...
			return (
				<Fragment>
					...
					<InspectorControls group="styles">
						<PanelBody title={__('Column Styling', 'sinag')}>
							<FormTokenField
								value={selectedStrings}
								label={__('For this column:', 'sinag')}
...
const modifySave = (props, block, attributes) => {
	...
	if (isAffected(name)) {
		...
		if (attributes.sinagClassNameColumn) {
			return {
				...props,
				className: classnames(
					props.className,
					attributes.sinagClassNameColumn.map((opt) => {
						return opt.value;
					})
				),
			};
