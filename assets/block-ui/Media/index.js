...
<Media
	...
	title={__('Background Media', 'sinag')}, // title above <MediaPlaceholder />
			...
			<PanelBody title={title}>
				...
				{getType(attributes[attributeNames.url]) !== 'video' && (
					<div style={{ marginTop: 10 }}>
						<TextareaControl
							label={__('Alt text (alternative text)', 'sinag')}
...
const MediaEdit = (props) => {
	...
	return show ? (
		<>
			<div className="sinag-media-remove">
				<ButtonX
					label={__('Remove Media', 'sinag')}
					handleClick={() =>
						setAttributes({
							[attributeNames.id]: '',
							[attributeNames.url]: '',
