<?php
/**
 * Work with blocks in sinag.
 * 
 * @package SinagCLI
 */

namespace SinagCLI\Block;
...
function block_create( $args, $assoc_args ) {
	...
	array_unshift( $far, array(
		...
		'repl' => ! $has_js ? "" : "wp_register_script( 'sinag-block-template-slug-script', get_template_directory_uri() . '/dist/block-library/template-slug/template-slug-script.js', null, '1.0', true );\n",
	) );
	array_unshift( $far, array(
		'find' => "\t/* PLACEHOLDER: add front-end script handle */\n",
		'repl' => ! $has_js ? "" : "\t'view_script_handles' => ['sinag-block-template-slug-script'],\n",
	) );
	array_unshift( $far, array(
	...
	array_unshift( $far, array(
		...
		'repl' => ! $has_inner_block ? " " : "\n\t\t\t\ttemplate={[['sinag/template-item-slug']]}\n\t\t\t\tallowedBlocks={['sinag/template-item-slug']}\n\t\t\t"
	) );
