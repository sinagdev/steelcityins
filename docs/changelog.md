...
## 6.0.2 - January 11, 2024
* Removed custom block patterns feature (this is now supported by core WordPress).
...
* Remove excerptView icon from blog-feed-edit.js
* Updated the sinag_custom_admin_footer()
* Fix the .card overflowing issue on the modals component
...
* Added Map component icons
* Added Logos and Icons fallback
...
* Reversed cli commands for robustness and clarity (e.g. `wp sinag bock create`).
...
* Change wp cli `wp sinag block` command to be `wp sinag create block` so it makes more sense.
* Added new wp cli `wp sinag create component` command to create basic components.
...
* Added password protected pattern and remove older year block pattern from wp cli `wp sinag setup` command.
...
* Fix `wp sinag setup` import path issue.
...
## 2.0.2 - April 25 2022
* Replace cached of sinag_id() since some hosts object cache the incorrect ID.

## 2.0.1 - December 23 2021
* Default to adding height/width to sinag_img() (disable by setting disable_size to true).
* Set sinag_img() to use full image size if not using srcset (e.g. for animated gif images).
...
* Fix function name for sinag_inner_images.
...
* Set default ID for sinag_img function.
...
* Added sinag class generating functions.
* Added common disambiguation function.
* Added media functions (sinag_bg, sinag_img, sinag_vid).
